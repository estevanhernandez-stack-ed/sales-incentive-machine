...
export function getGamesData(db: Database.Database) {
  ...
  const games = parseGames(contest.config_json).map((game) => {
    ...
    if (game.type === "sales_race") { const standings = servers.map((server) => ({ ...server, value: getMetric(db, goalMetricDefinition(game.metric), server.id, contest.id) })).sort((a, b) => b.value - a.value || a.id - b.id); return { ...game, metricLabel: metricLabel(game.metric, menuItems), awards, standings }; }
    const house = game.objectives.map((objective) => getHouseMetric(db, goalMetricDefinition(objective), contest.id));
    const standings = servers.map((server) => ({ ...server, objectives: game.objectives.map((objective, index) => { const value = getMetric(db, goalMetricDefinition(objective), server.id, contest.id); return { label: metricLabel(objective, menuItems), metric: objective.metric, value, complete: qualifiesForGoal(value, house[index], objective) }; }) }));
