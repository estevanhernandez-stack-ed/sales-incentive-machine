...
export function GameHub({ data }: { data: Data }) {
  const router = useRouter();
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState("");

  async function act(path: string, gameId: string) {
    setBusy(gameId);
    const response = await fetch(path, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameId }) });
    const result = await response.json().catch(() => null) as { error?: string } | null;
    setBusy("");
    setMessage(response.ok ? "Awards locked from live sales data." : result?.error ?? "Could not award this game.");
  ...
  return <section className="games-grid">
    ...
    {data.games.map((game) => game.type === "sales_race" ? <article className="game-card race" key={game.id}>
      ...
      {!game.awards.length && <button className="button no-print" disabled={busy === game.id} onClick={() => act("/api/games/race/lock", game.id)}>Lock final standings & award entries</button>}
    </article> : <article className="game-card mission" key={game.id}>
      ...
      {!game.awards.length && <button className="button no-print" disabled={busy === game.id} onClick={() => act("/api/games/mission/award", game.id)}>Award servers who completed the board</button>}
    </article>)}
